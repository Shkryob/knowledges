...
class ImageResize {
    public static function resize($path, $options = array(), $fname = '') {
        ...
        $uploadDir = DS . 'img' . DS . 'uploaded' . DS;
        ...
        $fname = $width . 'x' . $height . '_' . $cropstring . '_' . $fname;
        $relfile = '/img/uploaded/' . $fname; // relative file
        $cachefile = WWW_ROOT . $uploadDir . $fname; // location on server

        if (!is_dir(WWW_ROOT . $uploadDir)) {
            mkdir(WWW_ROOT . $uploadDir);
        }
        ...
        if ($resize) {
            ...
            if ($crop) {
                $tempcrop = imagecreatetruecolor($cropvars[2] - $cropvars[0], $cropvars[3] - $cropvars[1]);
                if (function_exists("imagecreatetruecolor") && $tempcrop) {
                    imagealphablending($tempcrop, false);
                    imagealphablending($image, false);
                    imagecopyresampled($tempcrop,
                            $image,
                            0,
                            0,
                            $cropvars[0],
                            $cropvars[1],
                            $cropvars[2] - $cropvars[0],
                            $cropvars[3] - $cropvars[1],
                            $cropvars[2] - $cropvars[0],
                            $cropvars[3] - $cropvars[1]);
                } else {
                    $tempcrop = imagecreate($cropvars[2] - $cropvars[0], $cropvars[3] - $cropvars[1]);
                    imagecopyresized($tempcrop,
                            $image,
                            0, 
                            0,
                            $cropvars[0],
